...
public class Pawn extends ChessPiece {
	...
	public boolean[][] possibleMoves() {
		...
		if (getColor() == Color.WHITE) {
			p.setValues(position.getRow() - 1, position.getColumn()); // Move uma casa pra frente
			if (getBoard().positionExistis(p) && !getBoard().thereIsAPiece(p)) {
				mat[p.getRow()][p.getColumn()] = true;
			}

			p.setValues(position.getRow() - 2, position.getColumn()); // Move duas casas pra frente
			...
			p.setValues(position.getRow() - 1, position.getColumn() - 1); // Se tiver adversario na lateral pode mover
